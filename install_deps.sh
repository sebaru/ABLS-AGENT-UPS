...
if [ "$SOCLE" = "fedora" ]; then
  ...
  dnf install -y glib2-devel json-glib-devel abls-libs-devel abls-agent-libs-devel nut-devel nut-client
fi
...
if [ "$SOCLE" = "debian" ] || [ "$SOCLE" = "raspbian" ] || [ "$SOCLE" = "ubuntu" ]; then
  ...
  apt install -y abls-libs-dev abls-agent-libs-dev libglib2.0-dev libjson-glib-dev libupsclient-dev nut
fi
